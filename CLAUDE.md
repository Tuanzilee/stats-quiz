...
- **2026-06-12 SCU-114-03 對答案**((18)(19) 翻面,**勘誤帳 +1**):
  - answer `'X,O,O,X,X,O,O,O'` → `'X,O,X,O,X,O,O,O'`(官方答案 XOXOXOOO,(18)(19) 修正);(16/17/20/21/22/23) 不變。
  - **(18) X**(原 O):前次解讀為「兩個 A 水準下 B 的方向皆非下降 → 算同方向」。重新對齊:A1 下 B 持平(無效果)、A2 下 B 遞增 — 持平與遞增「型態根本不同」,不算「相同方向只是程度有差」→ 敘述錯 → X。
  - **(19) O**(原 X):前次解讀為「(18) 描述的是 A 調節 B,不是 B 調節 A → 方向寫反」。重新對齊:有交互作用時調節關係**雙向對稱**(A 調節 B ⟺ B 調節 A),「B 為調節因子、調節 A 對 DV 的影響」是同一件事的另一種有效描述 → O。
  - solution_steps 整段重寫 9 步(原 8 步重寫,末加「答:X,O,X,O,X,O,O,O」明示);concepts「調節效果」→「調節效果(雙向對稱)」;key_concepts / ai_hint 全對齊「調節雙向對稱」立場(原本主張「X 的效果隨 Y 變 ⇒ Y 是調節因子」單向 framing 已刪);primary_concept 維持 ANOVA。
  - sw.js `v2026-06-12-2` → `v2026-06-12-3`
  - 累計勘誤 33 → 34
- **2026-06-12 NCCU-109-05 對答案**(D → B,**勘誤帳 +1**):
  - answer `'D'` → `'B'`;驗算確認 (B) 145 為正解。設計為二因子完全受試者內(A 學習方式 a=3、B 噪音 b=2、n=30,每人跑遍 3×2=6 情境);題目假設「受試者本身的記憶能力 × 實驗操弄無交互作用」→ 採加法模型,把 A×S、B×S、AB×S 三組受試者×處理交互合併成單一殘差。df_殘差 = df_total − df_受試者 − df_A − df_B − df_AB = 179 − 29 − 2 − 1 − 2 = 145。等價驗算 (a−1)(n−1)+(b−1)(n−1)+(a−1)(b−1)(n−1) = 58+29+58 = 145 ✓。
  - 陷阱 (D) 174 = 誤當受試者間二因子(abn − ab = 180 − 6 = 174),忽略受試者個別差異 df=n−1=29 須先分離 → 錯。
  - solution_steps 整段重寫(6 步:設計判定 → df_total → 各 df → 加法模型 → 殘差 + 驗算 → 陷阱說明),原本 13 步 confused reasoning(算出 145 自己又懷疑「但答案選 D」「需重新計算」)整段刪掉;concepts 改為「二因子受試者內ANOVA / 殘差自由度 / 受試者×處理交互作用 / 加法模型」;key_concepts / ai_hint / formula_used 全對齊新立場(原本帶問號的算式全清掉);primary_concept 維持 ANOVA。
  - sw.js `v2026-06-11-1` → `v2026-06-12-1`
  - 累計勘誤 32 → 33
